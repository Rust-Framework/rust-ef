...
// Note: `HavingExpr`, `AggKind`, and `CompareOp` live in `having_expr.rs`.
// `GroupBy` and `HavingCondition` were removed as dead code (superseded by
// `Vec<String>` and `HavingExpr` respectively).
