...
| `.to_list()` | `Vec<T>` | 返回列表 |
| `.first()` | `T` | 首条，无结果报错 |
| `.first_or_default()` | `Option<T>` | 首条�?None |
| `.single()` | `T` | 唯一一条，多条报错 |
| `.single_or_default()` | `Option<T>` | 唯一�?None |
| `.count()` | `i64` | 计数 |
| `.any()` | `bool` | 是否存在 |
...
**应做�?*
- 所�?trait �?`I` 为前缀（`IDbContext`, `IEntityType`, `IDatabaseProvider`�?- 使用 `DbContext`（无需自定�?context 结构体）
- 通过 `add_dbcontext(|o| o.use_sqlite(...))` 注册
- 多数据库使用 `add_dbcontext_keyed("key", |o| ...)`
- �?DI 解析�?`Arc<dyn IDbContext>`

**不应做：**
- �?context 上定�?`DbSet<Blog>` 结构体字�?- �?`BelongsTo<T>` �?`HasMany<T>` 上加 `IEntityType` trait bound
- �?builder 结构体上�?`IEntityType` bound
